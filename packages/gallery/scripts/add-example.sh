...
usage() {
  ...
  echo "Accepts both .md (markdown) and .mdx output from recap."
  echo ""
  echo "  --dir       Directory containing .md or .mdx files from recap output"
...
if [[ "$EXT" == "mdx" ]]; then
  ...
  if [[ -n "$FIRST_RAW" ]]; then
    ...
    awk -v url="https://www.youtube.com/watch?v=$VIDEO_ID" '
      /^---$/ && seen { print; print ""; print "> **Source:** Generated by [Recap](https://github.com/respeak-io/recap) from [YouTube](" url ")"; seen=2; next }
      /^---$/ && !seen { seen=1 }
      { print }
    ' "$FIRST_RAW" > "$TMPFILE"
  ...
  if [[ -n "$FIRST_RAW" ]]; then
    ...
    echo "" >> "$TMPFILE"
    echo "> **Source:** Generated by [Recap](https://github.com/respeak-io/recap) from [YouTube](https://www.youtube.com/watch?v=$VIDEO_ID)" >> "$TMPFILE"
...
if [[ "$EXT" == "mdx" ]]; then
  # MDX input: transform frontmatter (sidebar_position → sidebar.order), add YouTube embed to first
  for f in "$DIR"/*.mdx; do
    BASENAME="$(basename "$f")"
...

    {
      ...
      if [[ "$POSITION" == "1" ]]; then
        ...
        echo ""
        echo "> Generated by [Recap](https://github.com/respeak-io/recap)"
      fi
